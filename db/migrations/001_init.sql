-- Full CalmLane schema (tables, indexes, triggers, RLS).
-- Safe to re-run: triggers and policies are dropped before recreate (e.g. after 002_repair_missing_tables.sql).
...
-- Triggers: drop first so 001 is safe to re-run after 002 or a partial apply.
...
create trigger set_mood_logs_updated_at before update on public.mood_logs
for each row execute function public.set_updated_at();

drop trigger if exists set_coping_tool_sessions_updated_at on public.coping_tool_sessions;
create trigger set_coping_tool_sessions_updated_at before update on public.coping_tool_sessions
for each row execute function public.set_updated_at();
...
-- Policies: drop first so re-runs do not fail (e.g. after 002/003).
...
create policy "mood_logs_update_own" on public.mood_logs for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create policy "coping_tool_sessions_select_own" on public.coping_tool_sessions for select using (auth.uid() = user_id);
create policy "coping_tool_sessions_insert_own" on public.coping_tool_sessions for insert with check (auth.uid() = user_id);
create policy "coping_tool_sessions_update_own" on public.coping_tool_sessions for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "coping_tool_sessions_delete_own" on public.coping_tool_sessions for delete using (auth.uid() = user_id);
